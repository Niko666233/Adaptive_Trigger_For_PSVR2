...
#nullable disable
        ...
        public void Stop()
        {
            ...
            m_stream?.Dispose();
            m_client?.Close(); // TcpClient.Dispose() 实际上是 public Close()，Dispose() 只是实现了 IDisposable
            ...
            try
            {
                ...
                while (m_running && !token.IsCancellationRequested)
                {
                    ...
                    if (readable && clientSocket.Available > 0)
                    {
                        ...
                        if (bytesRead < Marshal.SizeOf(typeof(CommandHeader)))
                        {
        ...
        private void HandleIpcCommand(byte[] pBuffer, int bytesReceived)
        {
            ...
            switch (header.type)
            {
                ...
                case ECommandType.ServerHandshakeResult:
                    {
                        if (header.dataLen == Marshal.SizeOf(typeof(CommandDataServerHandshakeResult)))
                        {
                            CommandDataServerHandshakeResult response = ByteArrayToStructure<CommandDataServerHandshakeResult>(pBuffer, Marshal.SizeOf(typeof(CommandHeader)));
                ...
                case ECommandType.ServerGazeDataResult:
                    {
                        ...
                        {
                            if (header.dataLen == Marshal.SizeOf(typeof(CommandDataServerGazeDataResult)))
                            {
                                CommandDataServerGazeDataResult response = ByteArrayToStructure<CommandDataServerGazeDataResult>(pBuffer, Marshal.SizeOf(typeof(CommandHeader)));
                        ...
                        {
                            if (header.dataLen == Marshal.SizeOf(typeof(CommandDataServerGazeDataResult2)))
                            {
                                CommandDataServerGazeDataResult2 response = ByteArrayToStructure<CommandDataServerGazeDataResult2>(pBuffer, Marshal.SizeOf(typeof(CommandHeader)));
        ...
        private void SendIpcCommand<T>(ECommandType type, T data = default) where T : struct
        {
            ...
            int dataLen = data.Equals(default(T)) ? 0 : Marshal.SizeOf(typeof(T));
            int bufferLen = Marshal.SizeOf(typeof(CommandHeader)) + dataLen;
            ...
            IntPtr headerPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(CommandHeader)));
            Marshal.StructureToPtr(header, headerPtr, false);
            Marshal.Copy(headerPtr, buffer, 0, Marshal.SizeOf(typeof(CommandHeader)));
            ...
            if (dataLen > 0)
            {
                ...
                Marshal.Copy(dataPtr, buffer, Marshal.SizeOf(typeof(CommandHeader)), dataLen);
        ...
        private void SendIpcCommand(ECommandType type)
        {
            ...
            int bufferLen = Marshal.SizeOf(typeof(CommandHeader));
            byte[] buffer = new byte[bufferLen];
